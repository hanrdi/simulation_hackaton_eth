...
# SUPG
up_nm1 = fd.Function(UP)
u_nm1, p_nm1 = up_nm1.subfunctions
...
res = (
    - 6.0 / 7.0 * fd.grad(u) * u
    - fd.grad(p)
    + nu * fd.div(fd.sym(fd.grad(u)) + fd.transpose(fd.sym(fd.grad(u))))
    - (
...
"""
Reference: 
1. https://www.sciencedirect.com/science/article/pii/S0955799720303313?via%3Dihub
2. https://en.wikipedia.org/wiki/Streamline_upwind_Petrov%E2%80%93Galerkin_pressure-stabilizing_Petrov%E2%80%93Galerkin_formulation_for_incompressible_Navier%E2%80%93Stokes_equations
"""
he = np.min([Lx/Nx, Ly/Ny])      # the length of the shortest edge in a mesh
#print(he)
#print(rhof.dat.data.mean())
rho_he = rhof.dat.data.mean()

tau_SU = (4 * fd.inner(u,u) / he ** 2 + (4 * nu / rho_he ** 2) **2 ) ** (-0.5)  
tau_PS = he * he / tau_SU

# Weak formular
F_fluid = (
    fd.inner(2.0 / Re * fd.sym(fd.grad(u)), fd.sym(fd.grad(v))) * fd.dx
    - fd.inner(ns_param * fd.dot(fd.sym(fd.grad(u)), u_nm1), v) * fd.dx
    - fd.inner(ns_param * fd.dot(fd.sym(fd.grad(u_nm1)), u), v) * fd.dx
    + fd.inner(ns_param * fd.dot(fd.sym(fd.grad(u_nm1)), u_nm1), v) * fd.dx
    #- fd.inner(6.0 / 7.0 * fd.grad(u) * u, v) * fd.dx
    - fd.div(u) * q * fd.dx
    - fd.div(v) * p * fd.dx
    + p0 * fd.dot(v, n) * fd.ds(INLET)
)

F_fluid += fd.inner(- 7.0 * tau_PS / 6.0 * fd.grad(q), res) * fd.dx
F_fluid += (
    fd.inner(tau_SU * fd.dot(fd.sym(fd.grad(v)), (u + u_nm1)/2), res) * fd.dx
            )
