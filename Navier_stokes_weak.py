...
#k = 2
#C_k = 60 - 2**(k-2)
#tau_SU = (fd.inner(u,u) / (h_tau * h_tau) + nu / h_tau ** 4) ** (-0.5)
#tau_PS = h_tau * h_tau / tau_SU
#tau_SU = 1.0
#tau_PS = 1.0
...
#F_fluid += fd.inner(fd.grad(q), res) *fd.dx
#F_fluid += fd.inner(7.0 * tau_PS / (rho * 6.0) * fd.grad(q), res) * fd.dx
#F_fluid += fd.inner(tau_SU * fd.sym(fd.grad(v)) * u, res) * fd.dx
